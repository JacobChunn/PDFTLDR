...
export default function Profile() {
    ...
    const [uploadProfilePhoto, setUploadProfilePhoto] = useState<File | null>(null);
    ...
    const confirmDeleteDocument = async (confirm: boolean) => {
        if (confirm && deletingDocumentId !== null) {
            try {
                ...
                setSavedDocuments(savedDocuments.filter(doc => doc.id !== deletingDocumentId));
                ...
                <div className="w-1/5 mx-auto flex flex-col" style={{ height: "65%" }}>
                    ...
                    <div className="mb-8 relative">
                        ...
                        <div className="flex items-center flex-col mb-6">
                            <div className="[background:var(--color-background-grey)] rounded-full flex items-center justify-center mb-3" style={{ width: "150px", height: "150px", borderRadius: "50%" }}>
                                <img src={uploadProfilePhoto ? URL.createObjectURL(uploadProfilePhoto) : "placeholder.png"}className="w-full h-full object-cover rounded-full" />
                            </div>
                            {/* Upload Photo Button */}
                            <label htmlFor="upload-photo" className="[background:var(--color-blue)] [color:var(--color-white)] font-bold rounded-md px-6 py-3 hover:bg-blue-600 transition duration-300 mb-2" style={{ whiteSpace: 'nowrap', cursor: 'pointer' }}>Upload Photo</label>
                            <input type="file" id="upload-photo" accept="image/*" style={{ display: "none" }} onChange={handlePhotoChange} />
                            {/* Remove Button */}
                            {uploadProfilePhoto && (
                                <button onClick={handleRemoveButtonClick} className="[color:var(--color-blue)] text-center block hover:underline">remove</button>
                            )}
                        </div>
                    </div>
                    {/* Grey line */}
                    ...
                    <div className="mt-6 pt-4 border border-gray-400">
                        ...
                        <div className="overflow-y-auto" style={{ maxHeight: 'calc(100vh - 400px)' }}>
                            ...
                            {savedDocuments.map((document, index) => (
                                <div key={index} className="border-t border-gray-400 mt-1 pt-1" style={{ height: '120px' }}>
                                    <div className="flex items-center">
                                        ...
                                        <div className="mt-10 w-full">
                                            <h3 className="font-bold">{new Date(document.date_created).toLocaleDateString('en-US')}</h3>
                                        </div>
                                        <div className="mt-10 w-full">
                    ...
                    {selectedDocument && (
                        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
                            <div className="bg-white p-8 rounded-lg w-2/3 relative">
                                <h3 className="text-2xl font-bold mb-8">{selectedDocument.file_name}</h3>
                                <p className="mb-10">{selectedDocument.summarized_text}</p>
                                <button className="absolute mt-6 px-4 py-2 bottom-4 right-4 bg-blue-500 text-white rounded-md transition duration-300 hover:bg-blue-600" onClick={handleCloseModal}>Close</button>
                            </div>
                        </div>
                    )}
                    {/* Confirmation Dialog */}
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50">
                            <div className="bg-white p-6 rounded-md">
                                ...
                                <div className="mt-4 flex justify-end">
                                    <button className="px-4 py-2 mr-4 border border-gray-300 rounded-md" onClick={() => confirmDeleteDocument(false)}>Cancel</button>
                                    <button className="px-4 py-2 bg-red-500 text-white rounded-md" onClick={() => confirmDeleteDocument(true)}>Delete</button>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className=" flex justify-center">
                        <ul className="flex gap-[30px]">
                            ...
                            <li>
                                <button><Link href="#">Help</Link></button>
                            </li>
                        </ul>
                    </div>
